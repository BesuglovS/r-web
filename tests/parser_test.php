...
/* ── extract_blocks: сдвинутая заголовочная строка соседней полосы ── *
 * Регрессия: заголовок вне-блочной полосы (5Ж справа от сетки) посреди
 * данных обрывал обработку остальных полос — у 5А оставался 1 урок.
 * Также: вне-блочная полоса с дублирующим классом не импортируется.      */
echo "extract_blocks (сдвинутый заголовок соседней полосы):\n";
$rows = [
    // r1: заголовок основного блока (5А + 5Ж)
    1 => [0 => 'ПОНЕДЕЛЬНИК', 2 => '5А', 12 => 'ПОНЕДЕЛЬНИК', 14 => '5Ж'],
    2 => [0 => '08:00-08:40', 1 => '1', 2 => 'Русский язык/Шкарина',
          12 => '08:00-08:40', 13 => '1', 14 => 'Математика/Нежданова'],
    // r3: второй «Русский» — вертикальный merge уже заполнен парсером
    3 => [0 => '08:50-09:30', 1 => '2', 2 => 'Русский язык/Шкарина',
          12 => '08:50-09:30', 13 => '2', 14 => 'Русский язык/Андреева'],
    // r4: данные основного блока + СДВИНУТЫЙ заголовок полосы «5Ж» (справа)
    4 => [0 => '09:50-10:30', 1 => '3', 2 => 'Литература/Шкарина',
          12 => '09:50-10:30', 13 => '3', 14 => 'История/Хайбрахманов',
          24 => 'ПОНЕДЕЛЬНИК', 26 => '5Ж'],
    5 => [0 => '10:50-11:30', 1 => '4', 2 => 'Немецкий/Верещагина',
          12 => '10:50-11:30', 13 => '4', 14 => 'Физкультура/Ильичев',
          24 => '08:00-08:40', 25 => '1', 26 => 'Математика/Нежданова'],
    6 => [0 => '11:40-12:20', 1 => '5', 2 => 'Информатика/Волков',
          24 => '08:50-09:30', 25 => '2', 26 => 'Русский язык/Андреева'],
    7 => [0 => '12:30-13:10', 1 => '6', 2 => 'Музыка/Финкельштейн'],
];
$blocks = extract_blocks($rows);
$by_class = [];
foreach ($blocks as $b) {
    $by_class[$b['class']] = ($by_class[$b['class']] ?? 0) + count($b['lessons']);
}
check('5А: все 6 уроков (заголовок чужой полосы не обрывает сетку)', ($by_class['5А'] ?? 0) === 6);
$ru5a = array_filter(array_merge(...array_map(fn($b) => $b['lessons'], array_filter($blocks, fn($b) => $b['class'] === '5А'))), fn($l) => mb_strpos($l['cell'], 'Русский') === 0);
check('5А: двойной урок (2 одинаковых «Русских»)', count($ru5a) === 2);
$fives = array_values(array_filter($blocks, fn($b) => $b['class'] === '5Ж'));
